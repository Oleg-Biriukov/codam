if __name__ == "__main__":
    '''
.different() is mean the elements in first set that are not in the second
.union() just merges two sets
.intersection() include the element that appeared in bot sets
'''
    p_achvmnts = {
    ...
    un_achvmnt = p_achvmnts['alice'].union(p_achvmnts['bob'],
                                           p_achvmnts['charlie'])
    print(f"All unique achievements: {un_achvmnt}")
    print(f"Total unique achievements: {len(un_achvmnt)}\n")
    com_achv = p_achvmnts['alice'].intersection(p_achvmnts['bob'],
                                                p_achvmnts['charlie'])
    ...
    print(f'''\nAlice is missing: {un_achvmnt.difference(p_achvmnts["alice"])}
Bob is missing: {un_achvmnt.difference(p_achvmnts["bob"])}
Charlie is missing: {un_achvmnt.difference(p_achvmnts["charlie"])}
          ''')
