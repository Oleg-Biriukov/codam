...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/08/20 03:55:51 by obirukov          #+#    #+#             */
/*   Updated: 2026/08/20 03:58:32 by obirukov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	heapify_up(t_heapq *h, int index, t_schdl _prioriy)
{
	...
	if (index && _prioriy(h->items[index], root))
		return (swap(&h->items[(index - 1) / 2], &h->items[index]),
			heapify_up(h, (index - 1) / 2, _prioriy));
}
